...
function filterStudentsByGrade(students, grade) {
  ...
  const filtered = students.filter((students) => students.grade === grade);
  filtered.forEach((student) => console.log(student.name));
...
function compareArrays(arr1, arr2) {
  arr1.sort((a, b) => a - b);
  arr2.sort((a, b) => a - b);
  const jsArr1 = JSON.stringify(arr1);
  const jsArr2 = JSON.stringify(arr2);
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    newArrMatrix.push(row);
  }
  console.table(newArrMatrix);
  return newArrMatrix;
